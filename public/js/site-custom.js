(function ($) {
    ...
    var custom_js = {
        ...
        //             date: nextyear, // TODO Date format: 07/20/2022 17:00:00
...
var currentTime = new Date();
  ...
  var maxDisplay = recentDonations.length;
  var interval = 10000;

  // showRecentDonation(currentDisplay);
  ...
  function showRecentDonation(ordernum) {
    var firstline = recentDonations[ordernum].firstline;
    var secondline = recentDonations[ordernum].secondline;
    var image = recentDonations[ordernum].image;
    
    var ordertime = new Date(recentDonations[ordernum].thirdline*1000);
    var currentTime = Date.now();
    var diff = currentTime - ordertime;
    var formattedDiff = formatTimeDiff(diff);

    var orderNotificationElement = '<div id="RecentDonation" class="recent-donation animate__animated animate__fadeInLeft">'+
                                      '<div class="recent-donation__profile-icon" ><img src="'+image+'" alt="" class="img-thumbnail rounded-circle mb-2" width="100">'+
                                      '</div>'+
                                      '<div class="recent-donation__details">'+
                                        '<p class="recent-donation__firstline">'+firstline+'</p>'+
                                        '<p class="recent-donation__secondline">'+secondline+'</p>'+
                                        '<p class="recent-donation__thirdline">'+formattedDiff+'</p>'+
                                      '</div>'+
                                        '<div class="recent-donation__actions">'+
                                          '<i id="muteRecentDonation" class="fa fa-times"></i>'+
                                        '</div>'+
                                    '</div>';
    
    if($("#RecentDonation")) {
      dismissRecentDonation();
      setTimeout(function() {
        $("#RecentDonation").detach();
  ...
  function dismissRecentDonation() {
    $("#RecentDonation").removeClass("animate__fadeInLeft").addClass("animate__fadeOutDown");
  }

  function appendOrder(element) {
    $("body").append(element);
    $("#muteRecentDonation").on("click", function(){
